...
Reinforcement learning algorithms can broadly be categorized into \textit{online} and \textit{offline} methods depending on how training data is obtained. In online reinforcement learning, the agent learns by continuously interacting with the environment and collecting new data during training. The policy is therefore updated using trajectories generated by the current policy, allowing the agent to explore the state–action space and improve its behavior over time. Algorithms such as \gls{ppo}, \gls{sac}, and \gls{ac3} fall into this category and have demonstrated strong performance in continuous control and robotics applications.

In contrast, offline reinforcement learning (also known as batch reinforcement learning) assumes that the agent is trained using a fixed dataset of previously collected trajectories without further interaction with the environment. This setting arises in many practical applications where environment interaction is expensive, unsafe, or impossible, such as robotics, healthcare, and scientific simulations. Standard online algorithms often perform poorly in this regime because the learned policy may select actions outside the distribution of the dataset, leading to unreliable value estimates. Consequently, specialized algorithms such as \gls{cql}, \gls{iql}, and \gls{td3bc} have been developed to mitigate distribution shift and overestimation errors by constraining policy updates or learning conservative value functions.
...
where $\pi_\theta(a|s)$ is the policy and $\hat{A}_t$ is an estimator of the advantage function~\cite{schulman2017}. This formulation encourages the policy to increase the probability of actions that yield positive advantages and decrease the probability of actions with negative advantages.

However, performing multiple gradient updates on the same data can lead to excessively large policy updates, resulting in unstable learning. Earlier algorithms such as \gls{trpo} addressed this by constraining the \gls{kl} divergence between successive policies, but solving this constrained optimization problem is computationally expensive.
...
Training proceeds iteratively by collecting trajectories using the current policy, estimating advantages, and then performing several epochs of minibatch stochastic gradient ascent on the surrogate objective before collecting new data. This allows \gls{ppo} to reuse sampled data efficiently while maintaining stable policy improvement.

Empirically, \gls{ppo} achieves performance comparable to or better than \gls{trpo} while being significantly easier to implement and more computationally efficient. It has become one of the most widely used reinforcement learning algorithms for continuous control and large-scale policy learning problems.

% a3c a2c
\gls{ac3} is an on-policy reinforcement learning algorithm that combines policy gradient methods with value function approximation. The method was introduced to improve the stability and efficiency of deep reinforcement learning by allowing multiple agents to interact with independent copies of the environment in parallel. Each worker maintains its own environment instance and periodically updates a shared set of global network parameters~\cite{mnih2016}.

A3C follows an actor–critic architecture in which a single neural network outputs both the policy $\pi_\theta(a|s)$ (actor) and a value estimate $V_\theta(s)$ (critic). The actor is trained using the policy gradient theorem with an advantage estimate, while the critic learns to approximate the state-value function through regression. The policy gradient update can be written as
...
where $A_t$ is the advantage function, typically estimated using the difference between the observed return and the predicted value function. The advantage formulation reduces variance in the gradient estimate compared to using raw returns.

In A3C, multiple worker agents run in parallel and independently collect trajectories from their environments. Each worker computes gradients using its local experiences and asynchronously applies these updates to the shared global network. This asynchronous training scheme decorrelates the data collected by different workers and stabilizes learning without the need for a replay buffer.
...
Empirically, A3C demonstrated strong performance across a wide range of reinforcement learning benchmarks, including Atari and continuous control tasks, while significantly reducing training time through parallelization. The algorithm became one of the first scalable deep reinforcement learning methods and laid the foundation for many subsequent actor–critic algorithms~\cite{mnih2016}.
...
Because of this pessimistic value estimation, \gls{cql} is particularly robust when the dataset is noisy, narrow, or partially random. Empirical results demonstrate that \gls{cql} consistently outperforms several earlier offline \gls{rl} algorithms, and behavior cloning baselines on a range of benchmark tasks~\cite{kumar2020}. The trade-off is that the conservative regularization increases computational cost and may introduce some degree of underestimation bias.
